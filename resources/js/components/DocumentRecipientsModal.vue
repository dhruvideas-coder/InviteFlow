            ...
            <div v-if="modelValue" class="fixed inset-0 z-50 flex flex-col bg-slate-50 overflow-hidden">
                    ...
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-5">
                        ...
                        <div class="flex items-stretch bg-white rounded-2xl border border-gray-100 shadow-sm divide-x divide-gray-100 overflow-hidden">
                            <div class="flex-1 flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-4 py-2 sm:py-2.5 min-w-0">
                                <div class="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-primary-50 flex items-center justify-center shrink-0">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                                    </svg>
                                </div>
                                <div class="min-w-0 leading-tight">
                                    <p class="text-base sm:text-xl font-bold text-gray-900">{{ pagination.total }}</p>
                                    <p class="text-[10px] sm:text-xs text-gray-500 truncate">{{ lang.t('total_recipients') }}</p>
                                </div>
                            </div>
                            <div class="flex-1 flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-4 py-2 sm:py-2.5 min-w-0">
                                <div class="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-emerald-50 flex items-center justify-center shrink-0">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <div class="min-w-0 leading-tight">
                                    <p class="text-base sm:text-xl font-bold text-gray-900">{{ globalStats.total_sent }}</p>
                                    <p class="text-[10px] sm:text-xs text-gray-500 truncate">{{ lang.t('links_sent') }}</p>
                                </div>
                            </div>
                            <div class="flex-1 flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-4 py-2 sm:py-2.5 min-w-0">
                                <div class="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-amber-50 flex items-center justify-center shrink-0">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <div class="min-w-0 leading-tight">
                                    <p class="text-base sm:text-xl font-bold text-gray-900">{{ globalStats.total_pending }}</p>
                                    <p class="text-[10px] sm:text-xs text-gray-500 truncate">{{ lang.t('pending') }}</p>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <tr v-for="r in recipients" :key="r.id">
                                            ...
                                            <td>
                                                <div class="flex items-center gap-1 justify-end">
                                                    <!-- Pending: WhatsApp opened, waiting for the sender to confirm -->
                                                    <template v-if="rowState(r) === 'pending'">
                                                        <button @click="confirmSent(r)" :disabled="rowBusy(r)" class="btn btn-sm bg-green-600 hover:bg-green-700 text-white gap-1 disabled:opacity-50" :title="lang.t('confirm_sent_hint')">
                                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                                            {{ lang.t('btn_sent') }}
                                                        </button>
                                                        <button @click="markNotSent(r)" :disabled="rowBusy(r)" class="btn btn-sm btn-secondary gap-1 disabled:opacity-50" :title="lang.t('confirm_not_sent_hint')">
                                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                                            {{ lang.t('btn_not_sent') }}
                                                        </button>
                                                    </template>
                                                    <!-- Sent / not-yet-sent: WhatsApp button hidden once sent or capped -->
                                                    <template v-else>
                                                        <span v-if="rowState(r) === 'sent'" class="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100" :title="lang.t('whatsapp_already_sent')">
                                                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                                            {{ lang.t('sent') }}
                                                        </span>
                                                        <button v-else-if="!blocked" @click="sendWhatsApp(r)" :disabled="sendingIds.has(r.id)" class="btn btn-ghost btn-sm text-green-600 hover:text-green-700 hover:bg-green-50 disabled:opacity-50" title="Send WhatsApp">
                ...
                <Teleport to="body">
                    <Transition name="toast">
                        <div v-if="toastMsg" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[70] max-w-[90vw]">
                            <div
                                class="flex items-center gap-2.5 px-4 py-3 rounded-2xl shadow-lg text-sm font-semibold text-white"
                                :class="toastKind === 'warn' ? 'bg-amber-600' : 'bg-emerald-600'"
                            >
                                <svg v-if="toastKind === 'warn'" class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M5.07 19h13.86c1.54 0 2.5-1.67 1.73-3L13.73 4a2 2 0 00-3.46 0L3.34 16c-.77 1.33.19 3 1.73 3z"/></svg>
...
// ── WhatsApp direct send + Sent/Not-Sent confirmation ──────────────────
const selectedRecipient = ref(null);
const sendingIds        = ref(new Set());
const busyIds           = ref(new Set());
const copiedToken       = ref(null);

// Per-row WhatsApp links for the current document.
function docWhatsappLinks(r) {
    const docId = props.document?.id;
    return (r.invitation_links || []).filter(l => l.document_id === docId && l.via === 'WhatsApp');
}
function pendingLink(r) {
    return docWhatsappLinks(r).find(l => !l.confirmed_at) || null;
}

/** 'pending' (awaiting Sent/Not-Sent), 'sent' (confirmed), or 'none'. */
function rowState(r) {
    if (pendingLink(r)) return 'pending';
    if (docWhatsappLinks(r).some(l => l.confirmed_at)) return 'sent';
    return 'none';
}
function rowBusy(r) { return busyIds.value.has(r.id); }
...
async function sendWhatsApp(recipient) {
    if (!props.document || blocked.value || sendingIds.value.has(recipient.id) || rowState(recipient) !== 'none') return;
    ...
    try {
        ...
        // Link is created "pending" — it does NOT count yet and the recipient
        // is not marked sent until the user confirms via the Sent button.
        ...
        if (idx !== -1) {
            ...
            recipients.value[idx].invitation_links.unshift({
                id: data.id, token: data.token, created_at: data.created_at,
                document_id: doc.id, via: 'WhatsApp', confirmed_at: null, document: doc,
            });
        }
    } catch (err) {
...
// User confirms the WhatsApp message actually went out → count it.
async function confirmSent(recipient) {
    const link = pendingLink(recipient);
    if (!link || rowBusy(recipient)) return;
    setBusy(recipient.id, true);
    try {
        const { data } = await axios.post(`/api/invitation-links/${link.id}/confirm`);
        setQuota(data.whatsapp_quota);
        link.confirmed_at = data.confirmed_at || new Date().toISOString();
        recipient.sent = true;
        toast(lang.t('whatsapp_sent_toast', { used: quota.value.used, limit: quota.value.limit }));
    } catch (err) {
        if (err.response?.status === 429) {
            setQuota(err.response.data?.whatsapp_quota);
            toast(lang.t('whatsapp_limit_reached_title'), 'warn');
        } else { throw err; }
    } finally {
        setBusy(recipient.id, false);
    }
}

// User didn't send it → discard the pending link and restore the 3 buttons.
async function markNotSent(recipient) {
    const link = pendingLink(recipient);
    if (!link || rowBusy(recipient)) return;
    setBusy(recipient.id, true);
    try {
        await axios.delete(`/api/invitation-links/${link.id}`);
        recipient.invitation_links = (recipient.invitation_links || []).filter(l => l.id !== link.id);
    } finally {
        setBusy(recipient.id, false);
    }
}
